...
function quantitativaContinua() {
	...
	while (valorInicial <= xmax) {
		...
		var mediana1 = limiteInferior1 + ((medianaPosicao - facAnterior) / frSimples) * ic.toFixed(3)
		var mediana = limiteInferior1 + ((medianaPosicao - facAnterior1) / frSimples1) * ic.toFixed(3)
...
function calc_binomial(n) {
	...
	for (let i = 0; i < k.length; i++) {
		...
		let probabilidade = ((vetCalc[i] * (p ** k[i]) * (q ** (n - k[i]))) * 100).toFixed(2);
	...
	//probabilidade
	result_binomial[1] = totProbabilidade.toFixed(2)
	//desvio padrão
...
function calc_normal() {
	...
	*/
	...
	if (value == 0 || value == 1) {
		...
		let corretor = Math.floor(scoreZ * 10)
		if (scoreZ < 0){ scoreZ *= -1 ; corretor *= -1 ; x*= -1}
		if (x > corretor) x--
		...
		let x1 = (scoreZ1 * 10).toFixed(0)
		let corretor2 =Math.floor(scoreZ2 * 10)
		let corretor1 = Math.floor(scoreZ1 * 10)

		if (x1 > corretor1) { x1 = x1 - 1 }

		let x2 = (scoreZ2 * 10).toFixed(0)
